...
<head>
    ...
    <style>
        ...
        #questao:hover, #questao:hover, .alternative .options:hover {
            background-color: #bb2424;
            box-shadow: 3px 3px 2px 1px #460000;
        }
    </style>
    <script>
...
<body onload="focusMessage(1000), createStorage(), verifyMouse(), CorrectAnswerAlternative(barema, limite)">
    <div class="nav-buttons">
        <img class="btn-back" tabindex="2" src="{{ url('/imagens/Icon_back.png') }}" alt="Botão Voltar" onclick="direct('/conteudos/Porcentagem')" onkeypress="direct('/conteudos/Porcentagem')"/>
    ...
    <div class="painel-conteudos">
        <div class="painel-title no-focus-style">
            <div class="header-quest  col-12 col-xl-6 col-lg-7">
                <h1 id="title" class="title col-6">Porcentagem</h1>
        ...
        <div class="painel-body">
            <div class="col-esquerda col-xl-2 col-lg-1">
                @if($dados['fase'] > 1):
                    <img id="seta_esquerda" class="seta_pagina" tabindex="1" src="{{ url('/imagens/seta_pagina.png') }}" alt="Voltar questão anterior" onclick="<?= "direct('/conteudos/Porcentagem/". intval($dados['fase']-1) ."')" ?>" onkeypress="<?= "direct('/conteudos/Porcentagem/". intval($dados['fase']-1) ."')" ?>"/>
                @endif
            </div>
            ...
            <div class="col-direita col-xl-2 col-lg-1">
                @if($dados['fase'] < $dados['limite']):
                    <img id="seta_direita" class="seta_pagina" tabindex="1" src="{{ url('/imagens/seta_pagina.png') }}" alt="Pular questão" onclick="<?= "direct('/conteudos/Porcentagem/". intval($dados['fase']+1) ."')" ?>" onkeypress="<?= "direct('/conteudos/Porcentagem/". intval($dados['fase']+1) ."')" ?>" />
                @endif
            </div>
        </div>
    </div>
